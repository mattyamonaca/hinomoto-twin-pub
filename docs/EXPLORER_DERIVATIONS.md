...
## 仮定と文献の表示（2026-09-07）
M1〜M6の仮定は、確認できた文献が個々の仮定を直接支持するかを区別する。現行の地域への形状移植、条件付き独立、係数、端数混合などは本プロジェクトの判断なので「独自仮定」と表示する。「論文が存在しない」という断定ではなく、直接の裏付けを確認していないという意味である。統計の区分や未観測範囲をどう扱うかもここでは設計判断に含める。

`MODEL_LITERATURE` に書誌・リンク・確認範囲・支持する内容・支持しない内容を保存し、`MODEL_EVIDENCE` で各Mに対応させる。IPFの参考はLovelaceほか（2015、JASSS 18(2)21、本文§1.11–1.15・抄録確認）。所得割当ては杉浦ほか（2019、10.5687/iscie.32.69）、世帯は原田ほか（2018、10.9746/sicetr.54.705）、勤務地は堀上・村田（2026、10.9746/sicetr.62.228）の公開抄録を確認した。これらを本実装の独自仮定や係数の直接的根拠として扱わない。論文リンクと適用範囲は各Mの「手法の参考論文と適用範囲」に表示する。

追加の論文を採用する際は、該当節・対象母集団・仮定・本モデルへの適用範囲を確認してから分類を変更する。論文の手法を参考にしたことと、その論文が日本の当該条件付き分布を実証したことを混同しない。数値モデル・配布データは今回変更しない。
