...
画面は `data/graph.json` を非同期に読み込みます。形式は `schema_version: 1`、`dataset_version`、`graph` のオブジェクトです。`?data=https://example.org/graph.json` で別の配信先も指定できます（別ドメインでは配信元のCORS設定が必要）。読み込み失敗時は画面に案内を表示します。
