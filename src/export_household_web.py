...
def summarize(code):
    ...
    hh=pd.DataFrame({'hsize':g['size'].first(),'family':g['family'].first(),'has_spouse':g['role'].apply(lambda r:(r==1).any()),'child_u18':g.apply(lambda x:((x.role==2)&(x.age18<=3)).any()),'has_parent':g['role'].apply(lambda r:r.isin([4,5]).any()),'has_65':g['age18'].apply(lambda a:(a>=13).any()),'has_child_any':g['role'].apply(lambda r:(r==2).any())})
    ...
    for s in range(2):
        for a in range(18):
            ...
            rows.append({'n':int(n),'alone':float((x['hsize']==1).mean()),'with_spouse':float(x.has_spouse.mean()),'with_child_u18':float(x.child_u18.mean()),'with_child_any':float(x.has_child_any.mean()),'with_parent':float(x.has_parent.mean()),'with_65':float(x.has_65.mean()),'is_head':float((x.role==0).mean()),'size':[float((x['hsize']==k).mean()) for k in range(1,7)]+[float((x['hsize']>=7).mean())]})
    ...
    out={'code':code,'households':int(len(hh)),'members':int(len(df)),'members_15plus':int((df.age18>=3).sum()),'members_under_15':int((df.age18<3).sum()),'expected_households':float(N.sum()),'family_type':fam,'family_codes':hs.F,'family_labels':hs.FL,'age_bands':hs.A18L,'by_sex_age':rows,
         'note':'一般世帯（施設等の世帯を除く）の整数個票（seed 1）から集計。分母は各行の構成員数 n。世帯構成は2020年国勢調査の世帯表から生成した推定で、構成員の所得・学歴とは独立に抽出している。'}
    return out
