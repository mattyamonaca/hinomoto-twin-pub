...
def main():
 ...
 result={'passed':True,'model_version':json.loads((OUTPUT/'model_metadata.json').read_text()).get('model_version','2.0') if (OUTPUT/'model_metadata.json').exists() else '2.0','shape':list(shape),'csv_rows_verified':offset,'max_old_age_income_count_difference':difference,'max_old_age_sex_income_count_difference':sex_difference,'max_census_education_target_count_error':max_edu_error,'checks':['nonnegative finite counts','all v1 municipal age/income margins','all sex-specific income margins','education targets independently reconstructed','parent/ward aggregation','all CSV coordinates and probabilities','representative JSON consistency','conditioned sample fields and reproducibility','zero population and invalid filters','income association differs by education'],'validation_scope':'Internal/source consistency. No independent municipal education-income accuracy validation.'}
