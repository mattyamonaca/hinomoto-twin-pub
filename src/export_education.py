...
def main():
 ...
 version=json.loads((OUT/'model_metadata.json').read_text()).get('model_version','2.0') if (OUT/'model_metadata.json').exists() else '2.0'
 ...
 result={'model_version':version,'geographies':len(codes),'shape_per_municipality':[13,2,8,16],'csv_rows':total_rows,'defined_municipalities':int((population>0).sum()),'max_joint_sum_error':max_error,'compressed_csv_bytes':csv_path.stat().st_size}
