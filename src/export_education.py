...
import numpy as np
import pandas as pd
from build import BASE,OUT,AGES
...
def main():
 ...
 shutil.copyfile(BASE/'sources/education/education_bins.csv',OUT/'education_bins.csv')
 ...
 pd.DataFrame(examples,columns=['municipality_code','municipality_name','age_band','sex','education_code','education','estimated_population','p_age_sex_education_ge500_given_municipality','p_ge500_given_municipality_age_sex_education']).to_csv(BASE/'examples_v2.csv',index=False,float_format='%.10g')
 ...
 (BASE/'validation/education_export.json').write_text(json.dumps(result,indent=2));print(json.dumps(result,indent=2),flush=True)
